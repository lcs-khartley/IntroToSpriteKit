...
let actionWaitTwenty = SKAction.wait(forDuration: 20.0)
let rightThisMuch = CGVector(dx: 100, dy: 0)
let actionMoveRight = SKAction.move (by: rightThisMuch, duration: 0.5)
let actionWaitTwentyMoveRight = SKAction.sequence ([actionWaitTwenty,actionMoveRight])
circle1.run(actionWaitTwentyMoveRight)
